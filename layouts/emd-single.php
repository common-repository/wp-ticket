...
$container = apply_filters('emd_change_container','emd-wrapper','wp_ticket_com', 'single'); 
...
<?php 	$has_navigation = apply_filters( 'emd_show_temp_navigation', true, 'wp_ticket_com', 'single');
	...
	?>
		...
		<?php previous_post_link( '<div class="nav-previous">%link</div>', '<span class="meta-nav">' . _x( '<i class="fa fa-angle-left"></i>', 'Previous post link', 'wp-ticket-com' ) . '</span> %title' ); ?>
		<?php next_post_link( '<div class="nav-next">%link</div>', '%title <span class="meta-nav">' . _x( '<i class="fa fa-angle-right"></i>', 'Next post link', 'wp-ticket-com' ) . '</span>' ); ?>

		</nav>
<?php 	}
	endwhile; // end of the loop.
	$show_edit_link = apply_filters('emd_show_single_edit_link',true,'wp_ticket_com');
	if($show_edit_link){
		edit_post_link( esc_html__( 'Edit', 'wp-ticket-com' ) . ' <i class="fa fa-angle-right"></i>', '<div class="emd-edit-link" style="padding:20px 0;clear:both;text-align:left;">', '</div>' );
	}
	?>
</div>
