			...
			foreach ($matches[1] as $match_tag) {
				if(in_array($match_tag,Array('site_name','site_tagline','site_link'))){
					...
					foreach($mycomments as $mycomm){
						$new .= "<li><div>" . __("Author:","wp-ticket-com") . $mycomm->comment_author . "&nbsp;&nbsp;";
						$new .= get_comment_date(sprintf(__('%s \a\t %s', 'wp-ticket-com'),get_option('date_format'),get_option('time_format')),$mycomm->comment_ID) . "</div>";
...
if (!function_exists('emd_load_file')) {
	function emd_load_file() {
		...
		if ($ret === false) {
			echo '<div class="text-danger"><a href="' . wp_get_referer() . '">' . esc_html__('Please refresh the page and try again.', 'wp-ticket-com') . '</a></div>';
			die();
		}
