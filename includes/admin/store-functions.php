...
if (!function_exists('emd_display_store')) {
	function emd_display_store($appd) {
		...
		ob_start(); ?>
	<div class="wrap">
	<h2><?php echo esc_html($title);?> &nbsp;&mdash;&nbsp;<a href="https://emdplugins.com/plugins?pk_source=plugin-addons-page&pk_medium=plugin&pk_campaign=<?php echo esc_attr($appd);?>-addonspage&pk_content=browseall" class="button-primary" title="<?php esc_html_e( 'Browse All', 'wp-ticket-com' ); ?>" target="_blank"><?php esc_html_e( 'Browse All', 'wp-ticket-com' ); ?></a>
	</h2>
	<p><?php esc_html_e('The following plugins extend and expand the functionality of your app.','wp-ticket-com'); ?></p>
...
if (!function_exists('emd_display_support')) {
	function emd_display_support($appd,$show_review,$rev=''){
		...
		<div id="support-header"><?php printf(__('Thanks for installing %s.','wp-ticket-com'),constant(strtoupper($appd) . '_NAME'));?> &nbsp; <?php  printf(__('All support requests are accepted through <a href="%s" target="_blank">our support site.</a>','wp-ticket-com'),'https://emdplugins.com/support/?pk_source=support-page&pk_medium=plugin&pk_campaign=plugin-support&pk_content=supportlink'); ?>
	<?php 
		switch($show_review){
			case '1':
...
if (!function_exists('emd_display_review')) {
	...
	<div class="plugin-review-text"><a href="https://wordpress.org/support/view/plugin-reviews/<?php echo esc_attr($plugin); ?>" target="_blank"><?php esc_html_e('Like our plugin? Leave us a review','wp-ticket-com'); ?></a>
