...
class Emd_Notifications {
	...
	public function __construct($app) {
		...
		$this->notify_field_list = Array(
			...
			'ev_front_add' => array(
				'id' => 'ev_front_add',
				'name' => __('Frontend Add', 'wp-ticket-com') ,
				'info' => '',
				'type' => 'checkbox',
				'group' => 'event',
				'isset' => 1
			...
			'ev_back_add' => array(
				'id' => 'ev_back_add',
				'name' => __('Backend Add', 'wp-ticket-com') ,
				'info' => '',
				'type' => 'checkbox',
				'group' => 'event',
				'isset' => 1
			...
			'ev_change_val' => array(
				'id' => 'ev_change_val',
				'name' => __('Change Value', 'wp-ticket-com') ,
				'info' => __('If change value is empty, this notification is triggered in every value change.', 'wp-ticket-com') ,
				'type' => 'text',
				'isset' => 1
			) ,
			'user_msg' => array(
				'id' => 'user_msg',
				'name' => __('User Message', 'wp-ticket-com') ,
				'info' => '',
				'type' => 'header',
				'isset' => 0
			) ,
			...
			'user_from_name' => array(
				'id' => 'user_from_name',
				'name' => __('From Name', 'wp-ticket-com') ,
				'info' => '',
				'type' => 'text',
				'mtype' => 'user',
				'isset' => 0
			...
			'user_from_email' => array(
				'id' => 'user_from_email',
				'name' => __('From Email', 'wp-ticket-com') ,
				'info' => '',
				'type' => 'text',
				'mtype' => 'user',
				'isset' => 0
			...
			'user_reply_to' => array(
				'id' => 'user_reply_to',
				'name' => __('Reply To', 'wp-ticket-com') ,
				'info' => '',
				'type' => 'text',
				'mtype' => 'user',
				'isset' => 0
			...
			'admin_msg' => array(
				'id' => 'admin_msg',
				'name' => __('Admin Message', 'wp-ticket-com') ,
				'info' => '',
				'type' => 'header',
				'isset' => 0
			) ,
	...
	public function notify_static_callback($args) {
				...
				if(!empty($ent_list)){
					foreach ($ent_list as $kent => $vent) {
						...
						if($klist == 'attrs' && $vent['display_type'] == 'user'){
							$$klist.= sprintf(__('%s Login Username','wp-ticket-com'),esc_html($this->ent_list[$ment]['label'])) . ": <b>{" . esc_html($ment) . "_login_username}</b> ";
							$$klist.= sprintf(__('%s Login Password Reset Link','wp-ticket-com'),esc_html($this->ent_list[$ment]['label'])) . ": <b>{" . esc_html($ment) . "_login_password_reset_link}</b> ";
							$$klist.= sprintf(__('%s Verification Link','wp-ticket-com'),esc_html($this->ent_list[$ment]['label'])) . ": <b>{" . esc_html($ment) . "_verify_link}</b> ";
						}
					}
				}
				$badd = 1;
		...
		$globs = (empty($globs) ? __("Not available", 'wp-ticket-com') : rtrim($globs, ","));
		$attrs = (empty($attrs) ? __("Not available", 'wp-ticket-com') : rtrim($attrs, ","));
		$taxs = (empty($taxs) ? __("Not available", 'wp-ticket-com') : rtrim($taxs, ","));
		$rels = (empty($rels) ? __("Not available", 'wp-ticket-com') : rtrim($rels, ","));
		$coms = (empty($coms) ? __("Not available", 'wp-ticket-com') : rtrim($coms, ","));
		$fields = "<table><tr><th colspan=2>";
		$fields.= sprintf(__('Use template tags below to customize your email. Taxonomy and relationship tags produce link(s) to the related record(s). For no link tag, add %s to relationship or taxonomy tag. For example, for {mytag%s} tag produces a no link version of the tag. Check glossary tab for definitions.', 'wp-ticket-com') , '_nl', '_nl');
		$fields.= "</th></tr>";
		if ($this->notify_init_list[$args['notify_id']]['level'] == 'rel') {
			foreach ($ents as $ment) {
				$fields .= "<tr><th>" . esc_html($this->ent_list[$ment]['label']) . ' ' . __('Builtin', 'wp-ticket-com') . "</th>";
				$builtin = __('Title', 'wp-ticket-com') . ": <b>{" . esc_html($ment) . "_title}</b> " . __('Permalink', 'wp-ticket-com') . ": <b>{" . esc_html($ment) . "_permalink}</b> " . __('Edit Link', 'wp-ticket-com') . ": <b>{" . esc_html($ment) . "_edit_link}</b> " . __('Delete Link', 'wp-ticket-com') . ": <b>{delete_link}</b> " . __('Excerpt', 'wp-ticket-com') . ": <b>{" . esc_html($ment) . "_excerpt}</b> " . __('Content', 'wp-ticket-com') . ": <b>{" . esc_html($ment) . "_content}</b> " . __('Author Display Name', 'wp-ticket-com') . ": <b>{" . esc_html($ment) . "_author_dispname}</b> " . __('Author NickName', 'wp-ticket-com') . ": <b>{" . esc_html($ment) . "_author_nickname}</b> " . __('Author First Name','wp-ticket-com') . ": <b>{" . esc_html($ment) . "_author_fname}</b> " . __('Author Last Name','wp-ticket-com') . ": <b>{" . esc_html($ment) . "_author_lname}</b> " . __('Author Username','wp-ticket-com') . ": <b>{" . esc_html($ment) . "_author_login}</b> " . __('Author Bio','wp-ticket-com') . ": <b>{" . esc_html($ment) . "_author_bio}</b> " . __('Author Googleplus','wp-ticket-com') . ": <b>{" . esc_html($ment) . "_author_googleplus}</b> " . __('Author Twitter','wp-ticket-com') . ": <b>{" . esc_html($ment) . "_author_twitter}</b> ";
				$fields .= "<td>" . $builtin . "</td></tr>";
			}
		}
		else {
			$builtin = __('Title', 'wp-ticket-com') . ": <b>{title}</b> " . __('Permalink', 'wp-ticket-com') . ": <b>{permalink}</b> " . __('Edit Link', 'wp-ticket-com') . ": <b>{edit_link}</b> " . __('Delete Link', 'wp-ticket-com') . ": <b>{delete_link}</b> " . __('Excerpt', 'wp-ticket-com') . ": <b>{excerpt}</b> " . __('Content', 'wp-ticket-com') . ": <b>{content}</b> " . __('Author Display Name', 'wp-ticket-com') . ": <b>{author_dispname}</b> " . __('Author NickName', 'wp-ticket-com') . ": <b>{author_nickname}</b> " . __('Author First Name','wp-ticket-com') . ": <b>{author_fname}</b> " . __('Author Last Name','wp-ticket-com') . ": <b>{author_lname}</b> " . __('Author Username','wp-ticket-com') . ": <b>{author_login}</b> " . __('Author Bio','wp-ticket-com') . ": <b>{author_bio}</b> " . __('Author Googleplus','wp-ticket-com') . ": <b>{author_googleplus}</b> " . __('Author Twitter','wp-ticket-com') . ": <b>{author_twitter}</b> ";
			$fields .= "<tr><th>" . __('Builtin', 'wp-ticket-com') . "</th>";
			$fields .= "<td>" . $builtin . "</td></tr>";
		}

		$site_params = __('Site Title','wp-ticket-com') . " <b>{site_name}</b> " . __('Tagline','wp-ticket-com') . " <b>{site_tagline}</b> " . __('Site Address','wp-ticket-com') . " <b>{site_link}</b> ";
		$site_params = apply_filters('emd_notify_site_params',$site_params,$this->app,$ents);	
		$fields .= "<tr><th>" . __('Site', 'wp-ticket-com') . "</th><td>" . $site_params . "</td></tr>";
		

		$fields .= "<tr><th>" . __('Globals', 'wp-ticket-com') . "</th>
			<td>" . $globs . "</td>
			</tr>
			<tr><th>" . __('Attributes', 'wp-ticket-com') . "</th>
			<td>" . $attrs . "</td>
			</tr>
			<tr><th>" . __('Taxonomies', 'wp-ticket-com') . "</th>
			<td>" . $taxs . "</td>
			</tr>
			<tr><th>" . __('Relationships', 'wp-ticket-com') . "</th>
			<td>" . $rels . "</td>
			</tr>
			<tr><th>" . __('Comments', 'wp-ticket-com') . "</th>
			<td>" . $coms . "</td>
			</tr>
			</table>";
		$allowed_html = array(
	...
	public static function display_settings_notify($app, $notify_init_list) {
		if (!empty($notify_init_list)) {
			...
			echo '</h2>';
			echo '<p>' . esc_html__('Below is your notification list', 'wp-ticket-com') . ':</p>';
