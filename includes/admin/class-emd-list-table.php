...
class Emd_List_Table extends WP_List_Table {
		...
		if(!empty($this->has_bulk)){
			if($this->type == 'cfield'){
				$columns = array(
					...
					'name' => esc_html__( 'Name', 'wp-ticket-com' ),
					'label' => esc_html__( 'Label', 'wp-ticket-com' ),
					'entity' => esc_html__( 'Entity', 'wp-ticket-com' ),
					'ctype' => esc_html__( 'Type', 'wp-ticket-com' ),
					'dtype' => esc_html__( 'Display Type', 'wp-ticket-com' ),
					'created'   => esc_html__( 'Updated', 'wp-ticket-com' ),
				);
			}
			else {
				$columns = array(
					'cb'        => '<input type="checkbox" />',
					'name' => esc_html__( 'Name', 'wp-ticket-com' ),
					'type' => esc_html__( 'Type', 'wp-ticket-com' ),
					'shortcode' => esc_html__( 'Shortcode', 'wp-ticket-com' ),
					'created'   => esc_html__( 'Created', 'wp-ticket-com' ),
				);
				if($this->type == 'form'){
					$columns['modified'] = esc_html__( 'Modified', 'wp-ticket-com' );
				}
			}
		}
		else {
	...
	public function column_default( $rec, $column_name ) {
		switch ( $column_name ) {
			...
			case 'ctype':
				if($rec['ctype'] == 'attr'){
					$value = __('Attribute','wp-ticket-com');
				}
				elseif($rec['ctype'] == 'tax'){
					$value = __('Taxonomy','wp-ticket-com');
				}
				break;
			case 'dtype':
				$value = $rec['dtype'];
			...
			case 'shortcode':
				$value = '<p id="shortcode_' . $rec['id'] . '">' . stripslashes($rec['shortcode']) . '</p>
					<a class="emd-copy-clipb button button-primary" style="padding:1px 6px;" data-clipboard-target="#shortcode_' . $rec['id'] . '">' . __('Copy','wp-ticket-com') . '</a>';
				break;
			case 'created':
	...
	public function get_bulk_actions() {
		...
		if(!empty($this->has_bulk)){
			if($this->type == 'cfield'){
				$actions = array(
					'delete' => esc_html__( 'Delete Field', 'wp-ticket-com' ),
					'delete_data' => esc_html__( 'Delete Field & Data', 'wp-ticket-com' ),
				);
			}
			elseif($this->type == 'form'){
				$actions = array(
					'delete' => esc_html__( 'Delete', 'wp-ticket-com' ),
					'export' => esc_html__( 'Export', 'wp-ticket-com' ),
				);
			}
			else {
				$actions = array(
			...
			if (wp_verify_nonce( sanitize_text_field($_GET['_wpnonce']), 'bulk-' . $this->type . 's')){
				...
				<div class="notice updated">
					<p>
						<?php
						if ( count( $ids ) === 1 ) {
							esc_html_e( 'Record was successfully deleted.', 'wp-ticket-com' );
						} else {
							esc_html_e( 'Records were successfully deleted.', 'wp-ticket-com' );
						}
						?>
					</p>
				</div>
				...
				<div class="notice updated">
					<p>
						<?php esc_html_e( 'Security check failed. Please try again.', 'wp-ticket-com' ); ?>
					</p>
				</div>
				<?php
			}
	...
	/**
		...
		if($this->type == 'form'){
			$name = sprintf(
				...
				esc_html__('Edit Form', 'wp-ticket-com'),
				$name
			);
			...
			$row_actions['edit_settings'] = sprintf(
				...
				esc_html__('Edit Settings', 'wp-ticket-com'),
				esc_html__('Edit Settings', 'wp-ticket-com')
			);
			$row_actions['edit'] = sprintf(
				...
				esc_html__('Edit Layout', 'wp-ticket-com'),
				esc_html__('Edit Layout', 'wp-ticket-com')
			);
