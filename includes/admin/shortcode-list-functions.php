...
if (!function_exists('emd_show_shortcodes_page')) {
	function emd_show_shortcodes_page($app){
		...
		if(function_exists('emd_std_media_js') || function_exists('emd_analytics_media_js')){	
			echo '<span style="padding-right:10px;">' . esc_html__('Visual ShortCode Builder','wp-ticket-com') . '</span>'; 
			$has_bulk = 1;
			$create_url = admin_url('admin.php?page=' . $app . '_shortcodes#TB_inline?width=640&height=750&inlineId=wpas-component');
			echo '<a href="' . esc_url($create_url) . '" class="thickbox button button-primary">' . esc_html('Create New', 'wp-ticket-com') . '</a>';
			echo '</h2>';
			echo '<p>' . esc_html__('The following shortcodes are provided by default. To use the shortcode, click copy button and paste it in a page.','wp-ticket-com');
			echo ' ' . esc_html__('To create advanced shortcodes click Create New button.','wp-ticket-com') . '</p>';
		}
		else {
			echo '<span style="padding-right:10px;">' . esc_html__('ShortCodes','wp-ticket-com') . '</span>'; 
			echo '<a href="#" class="button button-primary btn-primary upgrade-pro">' . esc_html('Create New', 'wp-ticket-com') . '</a>';
			...
			echo '</h2>';
			echo '<p>' . esc_html__('The following shortcodes are provided by default. To use the shortcode, click copy button and paste it in a page.','wp-ticket-com');
			echo ' ' . sprintf(esc_html__('To learn more on how to create new shortcodes with filters go to the %s documentation.%s','wp-ticket-com'),'<a href="https://docs.emdplugins.com/docs/' . esc_attr(str_replace('_','-',$app)) . '" target="_blank">','</a>') . '</p>';
...
success : function(response){
	if(!response){
		alert('<?php echo esc_html__('Error: Please try again.','wp-ticket-com'); ?>');
	}
		...
		if(!empty($matches[1])){
			...
			if(!empty($myshc['name']) && !empty($shc_list['forms'])){
				if(array_key_exists($myshc['name'],$shc_list['forms'])){
					$myshc['type'] = __('Form','wp-ticket-com') . ' - ' . ucfirst($shc_list['forms'][$myshc['name']]['type']);
				}
			}
			if(empty($myshc['type']) && !empty($myshc['name']) && !empty($shc_list['shcs'])){
				if(array_key_exists($myshc['name'],$shc_list['shcs'])){
					$myshc['type'] = __('View','wp-ticket-com');
				}
			}
