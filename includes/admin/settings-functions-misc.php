	...
	function emd_misc_settings_tab($tabs,$app){
		...
		if(function_exists('emd_show_login_register_options') 
			...
			$show_tab = 1;
                ...
                if($show_tab == 1){
			$tabs['misc'] = __('Misc', 'wp-ticket-com');
	...
	<div class='tab-content' id='tab-misc' <?php if ( 'misc' != $active_tab ) { echo 'style="display:none;"'; } ?>>
		<?php	echo '<form method="post" action="options.php">';
			...
			$shc_list = get_option($app . '_shc_list');
				...
				echo esc_html__('Google Maps API Key','wp-ticket-com');
				echo '</label></th><td>';
				$google_mapkey = '';
				...
				echo "></input><p class='description'>" . esc_html__('Google requires api key to access google map service. By default we provide a complimentary key. Get your own google maps api key to avoid service interruptions','wp-ticket-com') . ": <a href='https://developers.google.com/maps/documentation/javascript/get-api-key' target='_blank'>Documentation</a></p></td></tr>";
                        }
			if(!empty($shc_list) && isset($shc_list['has_bs']) && $shc_list['has_bs'] == 1){
				echo "<tr><th scope='row'><label for='misc_settings_disable_bs_css'>";
				echo esc_html__('Disable Bootstrap CSS','wp-ticket-com');
				echo '</label></th><td>';
				$disable_bs_css =0;
				...
				echo "></input><p class='description'>" . esc_html__('Disables loading of Bootstrap stylesheet in plugin related pages when checked. You may need to disable it if your theme already uses Bootstrap based stylesheet.','wp-ticket-com') . "</p></td></tr>";
				echo "<tr><th scope='row'><label for='misc_settings_disable_bs_js'>";
				echo esc_html__('Disable Bootstrap JS','wp-ticket-com');
				echo '</label></th><td>';
				$disable_bs_js =0;
				...
				echo "></input><p class='description'>" . esc_html__('Disables loading of Bootstrap JavaScript in plugin related pages when checked. You may need to disable it if your theme already uses Bootstrap based JavaScript.','wp-ticket-com') . "</p></td></tr>";
			}
			if(!empty($shc_list) && isset($shc_list['has_autocomplete']) && $shc_list['has_autocomplete'] == 1){
				...
				echo esc_html__('Autocomplete Minimum Input Length','wp-ticket-com');
				echo '</label></th><td>';
				$min_input = 3;
				...
				echo esc_html__('Autocomplete Number of Results Returned','wp-ticket-com');
				echo '</label></th><td>';
				$num_result = 5;
			...
			if(function_exists('emd_show_login_register_options') && emd_show_login_register_options($app)){
				echo '<tr><th scope="row"><label for="misc_no_access_msg">' . esc_html__('No Access Message','wp-ticket-com') . '</label></th>';
				$no_access_msg = (isset($misc_settings['no_access_msg'])) ? $misc_settings['no_access_msg'] : __('You do not have sufficient permissions to access this page.','wp-ticket-com');
				...
				echo '</td></tr>';
				echo '<tr><th scope="row"><label for="misc_login_reg">' . esc_html__('Show Register / Login Form','wp-ticket-com') . '</label></th>';
				echo '<td colspan=5><select id="' . esc_attr($app) . '_misc_settings_login_reg" name="' . esc_attr($app) . '_misc_settings[login_reg]">';
				$login_reg_options = Array('none' => __('Only Message','wp-ticket-com'),
							'both' => __('Registration and Login Forms','wp-ticket-com'),
							'login' => __('Only Login Form','wp-ticket-com'));
				...
				echo '</select>';
				echo '<p class="description">' . esc_html__('Display the registration and login forms on the pages non-logged-in users don\'t have access to.','wp-ticket-com') . '</p></td></tr>';
			}
			echo '</tbody></table>';
			echo '</ul></div>';
			submit_button(); 
