...
function emd_show_login_register_forms($app,$fcontent,$show){
	if($show != 'none'){
		...
		if(!empty($_GET['emd_error'])){
			switch($_GET['emd_error']){
				case 'invalid_login':
					$error = __('Invalid username, email or password.','wp-ticket-com');
					break;
			}
		}
		else {
	...
	$log_reg_vars['user_email_msg'] = __('This email has been already registered.', 'wp-ticket-com');
	$log_reg_vars['verify_msg'] = __('Invalid username','wp-ticket-com');
	$log_reg_vars['verify_pass'] = __('Please enter at least 5 characters.','wp-ticket-com');
	$log_reg_vars['verify_pass2'] = __('Please enter same password.','wp-ticket-com');
	$log_reg_vars['verify_email'] = __('Email address already taken.','wp-ticket-com');
	...
	if($show != 'none'){
		...
		if(!empty($misc_settings['no_access_msg'])){
			...
			_e('You do not have sufficient permissions to access this page.', 'wp-ticket-com');
		}
		echo '</div></div>';
	}
}
...
function emd_login_register_verify_email(){
		...
		if(email_exists(sanitize_email($_POST['email']))) {
                        wp_send_json_error(array('msg' => __('Email aready exists','wp-ticket-com')));
                }
                else {
