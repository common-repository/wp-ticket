		...
		if(empty($shc_list['has_form_lite']) && function_exists('emd_form_builder_process')){
			...
			if (!empty($myform) && $_POST['form_name'] == $fcontent['name']) {
				if(!empty($fcontent['settings']['enable_ajax']) || !empty($_POST['save_step']) || !empty($_POST['save_end'])){
					...
					if ($ret === false) {
						$ret = '<div class="text-danger"><a href="' . wp_get_referer() . '">' . __('Please refresh the page and try again.', 'wp-ticket-com') . '</a></div>';
					...
					if(!empty($_POST['login_box_reg_username']) && !empty(trim($_POST['login_box_reg_password'])) && !empty(trim($_POST['login_box_reg_confirm_password']))){
						if(trim($_POST['login_box_reg_password']) == trim($_POST['login_box_reg_confirm_password'])){
							if(!validate_username(sanitize_text_field($_POST['login_box_reg_username']))){	
								wp_send_json_error(array('msg' => __('Invalid username','wp-ticket-com')));
								die();
							}
							elseif(username_exists(sanitize_text_field($_POST['login_box_reg_username']))){
								wp_send_json_error(array('msg' => __('Username already taken','wp-ticket-com')));
								die();
							}
							else {
								...
								if(is_wp_error($new_user_id)){
									wp_send_json_error(array('msg' => __('Please try again','wp-ticket-com')));
									die();
								}
							...
							$ret = '<div class="text-danger"><a href="' . wp_get_referer() . '">' . __('Please enter same password.', 'wp-ticket-com') . '</a></div>';
							wp_send_json_error(array('status' => 'error', 'msg' => $ret));
							die();
						}
					}
						...
						if(empty($_POST['login_box_reg_username'])){
							$ret = '<div class="text-danger"><a href="' . wp_get_referer() . '">' . __('Please enter username.', 'wp-ticket-com') . '</a></div>';
						}
						elseif(empty($_POST['login_box_reg_password'])){
							$ret = '<div class="text-danger"><a href="' . wp_get_referer() . '">' . __('Please enter password.', 'wp-ticket-com') . '</a></div>';
						}
						elseif(empty($_POST['login_box_reg_confirm_password'])){
							$ret = '<div class="text-danger"><a href="' . wp_get_referer() . '">' . __('Please enter confirm password.', 'wp-ticket-com') . '</a></div>';
						}
						wp_send_json_error(array('status' => 'error', 'msg' => $ret));
						die();
					}
	...
	switch($cfield['display_type']){
		...
		case 'select_advanced':
			...
			if(!empty($options)){
				foreach($options as $kopt => $vopt){
					...
					$attr_lay .= '>' . __($vopt,'wp-ticket-com') . '</option>';
				}
			}
			$attr_lay .= '</select>';
			break;
		...
		case 'thickbox_image':
			...
			if (!empty($cfield['max_files'])) {
				$attr_lay .= '<div class="emd-max-file-num">' . sprintf(__('Max number of files: %s', 'wp-ticket-com') , $cfield['max_files']) . '</div>';
			}
			if (!empty($cfield['max_file_size'])) {
				$attr_lay .= '<div class="emd-max-file-size">' . sprintf(__('Max file size: %s', 'wp-ticket-com') , $cfield['max_file_size']) . ' KB</div>';
			}
			if (!empty($cfield['file_exts'])) {
				$attr_lay .= '<div class="emd-max-file-exts">' . sprintf(__('File extensions allowed: %s', 'wp-ticket-com') , $cfield['file_exts']) . '</div>';
			}
			$attr_lay .= '</div>';
...
function emd_form_builder_lite_render_form($form_id,$app,$fcontent,$error='',$submit_result,$atts_set=''){
	...
	$local_vars['validate_msg']['required'] = __('This field is required.', 'wp-ticket-com');
	$local_vars['validate_msg']['passw'] = __('Please enter same password.', 'wp-ticket-com');
	$local_vars['validate_msg']['remote'] = __('Please fix this field.', 'wp-ticket-com');
	$local_vars['validate_msg']['email'] = __('Please enter a valid email address.', 'wp-ticket-com');
	$local_vars['validate_msg']['url'] = __('Please enter a valid URL.', 'wp-ticket-com');
	$local_vars['validate_msg']['date'] = __('Please enter a valid date.', 'wp-ticket-com');
	$local_vars['validate_msg']['dateISO'] = __('Please enter a valid date ( ISO )', 'wp-ticket-com');
	$local_vars['validate_msg']['number'] = __('Please enter a valid number.', 'wp-ticket-com');
	$local_vars['validate_msg']['digits'] = __('Please enter only digits.', 'wp-ticket-com');
	$local_vars['validate_msg']['creditcard'] = __('Please enter a valid credit card number.', 'wp-ticket-com');
	$local_vars['validate_msg']['equalTo'] = __('Please enter the same value again.', 'wp-ticket-com');
	$local_vars['validate_msg']['maxlength'] = __('Please enter no more than {0} characters.', 'wp-ticket-com');
	$local_vars['validate_msg']['minlength'] = __('Please enter at least {0} characters.', 'wp-ticket-com');
	$local_vars['validate_msg']['rangelength'] = __('Please enter a value between {0} and {1} characters long.', 'wp-ticket-com');
	$local_vars['validate_msg']['range'] = __('Please enter a value between {0} and {1}.', 'wp-ticket-com');
	$local_vars['validate_msg']['max'] = __('Please enter a value less than or equal to {0}.', 'wp-ticket-com');
	$local_vars['validate_msg']['min'] = __('Please enter a value greater than or equal to {0}.', 'wp-ticket-com');
	$local_vars['unique_msg'] = __('Please enter a unique value.', 'wp-ticket-com');
	$local_vars['user_email_msg'] = __('This email has been already registered.', 'wp-ticket-com');
	if(!empty($fcontent['settings']['enable_ajax'])){
	...
	if($fcontent['type'] == 'submit' && !empty($ret_attrs['file_attrs'])){
		...
		foreach($ret_attrs['file_attrs'] as $myfattr){
			...
			$local_vars[$myfattr]['btnText'] = __('File Upload', 'wp-ticket-com');
			...
			$local_vars[$myfattr]['errorMsg'] = __('ERROR:', 'wp-ticket-com');
			$local_vars[$myfattr]['invalidExtError'] = __('Invalid file type.', 'wp-ticket-com');
			$local_vars[$myfattr]['sizeError'] = __('File size is greater than allowed limit.', 'wp-ticket-com');
			$local_vars[$myfattr]['maxUploadError'] = __('Maximum number of allowable file uploads has been exceeded.', 'wp-ticket-com');
	...
	if($page_count == 1){
		...
		if(!empty($fcontent['settings']['submit_button_label'])){
			...
			$layout .= __('Submit','wp-ticket-com');
		}
...
function emd_form_builder_lite_process_login(){
	$nonce_verified = wp_verify_nonce(sanitize_text_field($_POST['nonce']), 'emd_login_form');
		...
		//error
		$error = __('Please refresh the page and try again.','wp-ticket-com');
	}
	else {
		$error = "";
		...
		if($user_data) {
			...
			if(wp_check_password(sanitize_text_field($_POST['emd_user_pass']), $user_data->user_pass, $user_data->ID)) {
				...
				$error = __( 'The password or username you entered is incorrect.', 'wp-ticket-com');
			}
		} else {
			$error = __('The password or username you entered is incorrect.', 'wp-ticket-com');
		}
