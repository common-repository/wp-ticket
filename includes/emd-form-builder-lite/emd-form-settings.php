...
function emd_form_builder_lite_settings($type,$app,$form_id=0,$result = ''){
	...
	$tabs['general'] = __('General', 'wp-ticket-com');
	$tabs['display'] = __('Display', 'wp-ticket-com');
	$tabs['submit'] = __('Submissions', 'wp-ticket-com');
	if(empty($fcontent['type']) || (!empty($fcontent['type']) && $fcontent['type'] != 'search')){
		$tabs['confirm'] = __('Confirmations', 'wp-ticket-com');
	}
	$tabs['schedule'] = __('Scheduling', 'wp-ticket-com');
	$tabs['antispam'] = __('Anti Spam', 'wp-ticket-com');
	if(empty($fcontent['type']) || (!empty($fcontent['type']) && $fcontent['type'] != 'search')){
		$tabs['page'] = __('Pages', 'wp-ticket-com');
	}
	else {
		//this will be added when we add ability to create new search forms
		$tabs['search'] = __('Search','wp-ticket-com');
	}
	$tabs['code'] = __('Code', 'wp-ticket-com');
	...
	if($type == 'edit'){
		...
		echo '<h2>' . esc_html__('Form Settings:','wp-ticket-com') . ' ' . esc_html($myform->post_title) . '</h2>';
	}
	else {
		echo '<input type="hidden" name="stype" value="new">';
		echo '<input type="hidden" name="app" value="' . esc_attr($app) . '">';
		echo '<h2>' . esc_html__('Form Settings:','wp-ticket-com')  . '</h2>';
	}	
	if($result == 'error'){
		echo '<div class="is-dismissible notice error"><p><strong>' . esc_html__('Error when saving.','wp-ticket-com') . '</strong></p></div>';
	}
	elseif($result == 'success'){
		echo '<div class="updated is-dismissible notice"><p><strong>' . esc_html__('Saved successfully.','wp-ticket-com') . '</strong></p></div>';
	}
...
function emd_form_builder_lite_tab($type,$mytab,$app,$active_tab,$fcontent){
	switch($mytab){
		case 'general':
			$login_reg_options = Array('none' => __('None','wp-ticket-com'),
				'both' => __('Registration and Login Forms','wp-ticket-com'),
				'login' => __('Login Form only','wp-ticket-com'));
			$type_options = Array('submit' => __('Submit','wp-ticket-com'),
				'search' => __('Search','wp-ticket-com')
			);
			$disabled = false;
			...
			$tab_fields = Array('title' => Array('label' => __('Name','wp-ticket-com'), 'type' => 'text', 'required' => true),
			'type' => Array('label' => __('Type','wp-ticket-com'), 'type' => 'select', 'options' => $type_options, 'disabled' => $disabled, 'required' => true),
			'entity' => Array('label' => __('Entity','wp-ticket-com'), 'type' => 'select', 'options' => $ent_options, 'disabled' => $disabled, 'required' => true),
			'login_reg' => Array('label' => __('Show Register / Login Form','wp-ticket-com'), 'type' => 'select', 'options' => $login_reg_options,
					'desc'=>__('Displays or hides registration and login forms on this form for non-logged-in users.','wp-ticket-com')),
			'noaccess_msg' => Array('label' => __('No Access Message','wp-ticket-com'), 'type' => 'textarea', 'default' => __('You are not allowed to access to this area. Please contact the site administrator.','wp-ticket-com')),
			);	
			break;
		case 'display':
			$device_options = Array('desktops' => __("Desktops","wp-ticket-com"),
					'phones' => __("Phones","wp-ticket-com"),
					'tablets' => __("Tablets","wp-ticket-com"),
					'large_desktops' => __("Large Desktops","wp-ticket-com")
			);
			$placement_options = Array('top' => __("Top","wp-ticket-com"),
						'left' => __("Left","wp-ticket-com"),
						'inside' => __('Inside','wp-ticket-com'),
			);
			$size_options = Array('medium' => __("Medium","wp-ticket-com"),
						'small' => __("Small","wp-ticket-com"),
						'large' => __('Large','wp-ticket-com'),
			);
			$tab_fields = Array('targeted_device' => Array('label' => __('Targeted Device','wp-ticket-com'), 'type' => 'select', 'options' => $device_options),
					'label_position' => Array('label' => __('Label Placement','wp-ticket-com'), 'type' => 'select', 'options' => $placement_options,
						'desc' => __("Sets the field label position relative to the field input location. Options are Top,Left or Inside. Pick your label placement based on the space you have available for the form. Min 680px required for inside/top label placement with 3 column layout. If you enabled operators in your search form, you will need more space for multi-layout designs. You can always adjust the width css element of your form container when needed. Enabling operators will give access to all of your data so limiting access by role may always be a good idea.","wp-ticket-com")
						),
					'element_size' => Array('label' => __('Element Size','wp-ticket-com'), 'type' => 'select', 'options' => $size_options),		
					'display_inline' => Array('label' => __('Display Radios and Checkboxes Inline','wp-ticket-com'), 'type'=>'checkbox'),
					'fill_usermap' => Array('label' => __('Do not autofill Logged-in User Information','wp-ticket-com'), 'type'=>'checkbox'),
					'form_class' => Array('label' => __('Form Class','wp-ticket-com'), 'type'=>'text'),
				);		
			break;
		case 'submit':
			$status_options = Array('publish' => __('Publish','wp-ticket-com'),
						'draft' => __('Draft','wp-ticket-com'),
						'future' => __('Future','wp-ticket-com'),
						'private' => __('Private','wp-ticket-com'),
						'trash' => __('Trash','wp-ticket-com')
			);
			$button_options = Array('btn-standard' =>  __("Standard (White - #FFFFF)","wp-ticket-com"),
						'btn-primary' => __("Primary (Blue - #006DCC)","wp-ticket-com"),
						'btn-info' => __("Info (Light Blue - #49AFCD)","wp-ticket-com"),
						'btn-success' => __("Success (Green - #5BB75B)","wp-ticket-com"),
						'btn-warning' => __("Warning (Orange - #FAA732)","wp-ticket-com"),
						'btn-danger' => __("Danger (Red - #DA4F49)","wp-ticket-com"),
						'btn-inverse' => __("Inverse (Black - #363636)","wp-ticket-com"),
						'btn-link' => __("Link (Blue -  #0088CC)","wp-ticket-com"),
						'btn-custom' => __("Custom","wp-ticket-com"),
			);
			$button_size_options = Array('btn-std' =>  __("Standard","wp-ticket-com"),
							'btn-xlarge' => __("XLarge","wp-ticket-com"),
							'btn-large' => __("Large","wp-ticket-com"),
							'btn-small' => __("Small","wp-ticket-com"),
							'btn-mini' => __("Mini","wp-ticket-com"),
			);
			$fa_size_options = Array('' => __('Standard','wp-ticket-com'),
					'fa-lg' => __('Large','wp-ticket-com'),
					'fa-2x' => __('2x','wp-ticket-com'),
					'fa-3x' => __('3x','wp-ticket-com'),
					'fa-4x' => __('4x','wp-ticket-com'),
					'fa-5x' => __('5x','wp-ticket-com'),
					);
			$fa_pos_options = Array('left' => __('Left','wp-ticket-com'),
						'right' => __('Right','wp-ticket-com'),
					);
			if(empty($fcontent['type']) || (!empty($fcontent['type']) && $fcontent['type'] != 'search')){
			...
			break;
		case 'confirm':
			$confirm_options = Array('text' => __('Show text','wp-ticket-com'),
						'redirect' => __('Redirect','wp-ticket-com'),
						'form' => __('Go to another form','wp-ticket-com')
					);
			$after_options = Array('show' => __('Show Form','wp-ticket-com'),
						'hide' => __('Hide Form','wp-ticket-com'),
			);
			...
			$tab_fields = Array('confirm_method' => Array('label' => 'Confirmation Method', 'type' => 'select', 'options' => $confirm_options, 'upgrade' => true, 'disabled' => true),
					...
					'success_msg' => Array('label' => 'Success Message', 'type' => 'textarea', 'depend' => 'settings_confirm_method_text','default'=> __('Thanks for your submission.','wp-ticket-com')),
		...
		case 'schedule':
			...
			break;
		case 'antispam':
			$captcha_options = Array('never_show' => __('Never Show','wp-ticket-com'),
						'show_always' => __('Always Show','wp-ticket-com'),
						'show_to_visitors' => __('Visitors Only','wp-ticket-com'),
					);
			...
			break;
		case 'page':
			$wizard_options = Array('default' => __('Tabs','wp-ticket-com'),
						'arrows' => __('Arrows','wp-ticket-com'),
						'circles' => __('Circles','wp-ticket-com'),
						'dots' => __('Dots','wp-ticket-com'),
					);
			$wizard_toolbar_options = Array('bottom' => __('Bottom','wp-ticket-com'),
							'top' => __('Top','wp-ticket-com'),
							'both' => __('Both','wp-ticket-com')
						);
			$transition_options = Array('none' => __('None','wp-ticket-com'),
						'slide' => __('Slide','wp-ticket-com'),
						'fade' => __('Fade','wp-ticket-com')
					);
			$status_options = Array('publish' => __('Publish','wp-ticket-com'),
						'draft' => __('Draft','wp-ticket-com'),
						'future' => __('Future','wp-ticket-com'),
			...
			$tab_fields = Array('wizard_style' => Array('label' => __('Wizard Style','wp-ticket-com'), 'type' => 'select','options' => $wizard_options),
					'wizard_vertical' => Array('label' => __('Display Vertical Wizard Steps','wp-ticket-com'), 'type' => 'checkbox'),
					'wizard_toolbar' => Array('label' => __('Wizard Button Toolbar','wp-ticket-com'), 'type' => 'select','options' => $wizard_toolbar_options),
					'wizard_cancel_url' => Array('label' => 'Cancel Button Link', 'type' => 'text'),
					'wizard_trans_effect' => Array('label' => __('Wizard Transition Effect','wp-ticket-com'), 'type' => 'select','options' => $transition_options),
					'wizard_trans_speed' => Array('label' => 'Wizard Transition Speed', 'type' => 'text', 'size' => 'small', 'default' => 400),
					'wizard_save_step' => Array('label' => __('Save Wizard Step Data','wp-ticket-com'),'type'=>'checkbox'),
		...
		case 'search':
			...
			if(!empty($fcontent['source']) && $fcontent['source'] == 'plugin'){
				$temp_options = Array('simple_table' => __('Basic HTML Table','wp-ticket-com'),'adv_table'=>__('Advanced Datagrid Table','wp-ticket-com'),'cust_table' => __('Default','wp-ticket-com'));
			}
			else {
				$temp_options = Array('simple_table' => __('Basic HTML Table','wp-ticket-com'),'adv_table'=>__('Advanced Datagrid Table','wp-ticket-com'));
			}
			$tab_fields = Array(
					'ajax_search' => Array('label' => __('Enable Ajax','wp-ticket-com'), 'type' => 'checkbox', 'upgrade' => true, 'disabled' => true),
					'enable_operators' => Array('label' => __('Enable Search Operators','wp-ticket-com'), 'type' => 'checkbox', 'upgrade' => true, 'disabled' => true),
					'display_records' => Array('label' => __('Display Records On Page Load','wp-ticket-com'), 'type' => 'checkbox', 'upgrade' => true, 'disabled' => true),
					'result_templ' => Array('label' => __('Result Template Type','wp-ticket-com'), 'type' => 'select', 'options' => $temp_options, 'upgrade' => true, 'disabled' => true),
					'adv_search' => Array('label' => __('Search','wp-ticket-com'), 'type' => 'checkbox', 'depend' => 'settings_result_templ_adv_table', 'upgrade' => true, 'disabled' => true),
					'adv_click' => Array('label' => __('Click to Select','wp-ticket-com'), 'type' => 'checkbox', 'depend' => 'settings_result_templ_adv_table', 'upgrade' => true, 'disabled' => true),
					'adv_show_col' => Array('label' => __('Show Columns','wp-ticket-com'), 'type' => 'checkbox', 'depend' => 'settings_result_templ_adv_table', 'upgrade' => true, 'disabled' => true),
					'adv_show_export' => Array('label' => __('Show Export','wp-ticket-com'), 'type' => 'checkbox', 'depend' => 'settings_result_templ_adv_table', 'upgrade' => true, 'disabled' => true),
					'adv_show_toggle' => Array('label' => __('Show Toggle','wp-ticket-com'), 'type' => 'checkbox', 'depend' => 'settings_result_templ_adv_table', 'upgrade' => true, 'disabled' => true),
					'adv_show_all' => Array('label' => __('Show All Records','wp-ticket-com'), 'type' => 'checkbox', 'depend' => 'settings_result_templ_adv_table', 'upgrade' => true, 'disabled' => true),
					'adv_page_size' => Array('label' => __('Page Size','wp-ticket-com'), 'type' => 'text', 'depend' => 'settings_adv_show_all_0','size'=> 'small', 'upgrade' => true, 'disabled' => true),
					'adv_page_list' => Array('label' => __('Page List','wp-ticket-com'), 'type' => 'text', 'depend' => 'settings_adv_show_all_0','desc'=> __('Enter semicolon separated values such as 10, 20, 50, all','wp-ticket-com'), 'upgrade' => true, 'disabled' => true),
					'adv_maintain' => Array('label' => __('Maintain Selected on Page Change','wp-ticket-com'), 'type' => 'checkbox', 'depend' => 'settings_adv_show_all_0', 'upgrade' => true, 'disabled' => true),
					'result_fields' => Array('label' => __('Result Fields','wp-ticket-com'), 'type' => 'select','options' => $result_options, 'multiple' => true, 'order' => true, 'upgrade' => true, 'disabled' => true),
					'noresult_msg' => Array('label' => __('No Result Message','wp-ticket-com'), 'type' => 'textarea','default'=>__('Your search returned no results.','wp-ticket-com'))
					);
			break;
		case 'code':
			$tab_fields = Array('css_enq' => Array('label' => __('CDN CSS File','wp-ticket-com'), 'type' => 'textarea', 'desc' => __('Enter semicolon separated CSS file urls starting with http(s).','wp-ticket-com')),
					'js_enq' => Array('label' => __('CDN JS File','wp-ticket-com'), 'type' => 'textarea', 'desc' => __('Enter semicolon separated JavaScript file urls starting with http(s).','wp-ticket-com'))
			);
			break;
	}
