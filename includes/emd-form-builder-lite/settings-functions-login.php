...
function emd_login_settings_tab($tabs,$app){
	...
	if($has_login_shc == 1){
		$tabs['login'] = __('Login', 'wp-ticket-com');
...
?>
	...
	<?php	echo '<form method="post" action="options.php">';
		...
		echo "<h4>" . esc_html__('Login/Register:','wp-ticket-com') . ' ' . esc_html__('Use the options below to customize Login page.','wp-ticket-com');
		echo '</h4>';
		...
		echo esc_html__('Login Page','wp-ticket-com');
		echo '</label></th><td>';
		...
		echo '</select>';
		echo "<p class='description'>" . sprintf(esc_html__('Use [emd_login app="%s"] shortcode in your page. Attributes: ent="emd_customer,emd_vendor" show_reg=1 , reg_label="",reg_link="",show_lost=1  ','wp-ticket-com'),esc_html($app)) . "</p></td></tr>";
		echo "<tr><th scope='row'><label for='login_settings_pass_reset_subj'>";
		echo esc_html__('Password Reset Message Subject','wp-ticket-com');
		echo '</label></th><td>';
		...
		echo esc_html__('Password Reset Message','wp-ticket-com');
		echo '</label></th><td>';
		ob_start();
		...
		echo "<tr><th scope='row'><label for='login_settings_pass_reset_msg'>";
		echo '</label></th><td>';
		echo esc_html__('Use these template tags to customize your email: {username}, {password_reset_link}, {sitename}','wp-ticket-com'); 
		echo '</td></tr>';
		...
		if(!empty($cust_roles)){
			...
			echo esc_html__('User Role Redirects','wp-ticket-com');
			echo '</h3>';
			echo "<span class='description' style='padding-left:10px;color:#777;'>- " . esc_html__('Set a redirect url which users belonging to a role will be redirected to after a successful login or logout.','wp-ticket-com') . "</span></th></tr>";
			$ent_list = get_option($app . '_ent_list');
			foreach($cust_roles as $krole => $vrole){
				...
				echo sprintf(esc_html__('%s Login','wp-ticket-com'),esc_html($vrole));
				echo '</label></th><td>';
				...
				if(!empty($ent_roles[$krole])){
					echo '<p class="description">' . esc_html__('If left empty, user will be directed to the corresponding user entity page','wp-ticket-com') . '</p>';
				}
				else {
					echo '<p class="description">' . esc_html__('If left empty, user will be directed to the site\'s homepage','wp-ticket-com') . '</p>';
				}
				echo '</td></tr>';
				echo "<tr><th scope='row'><label for='login_settings_redirect_logout'>";
				echo sprintf(esc_html__('%s Logout','wp-ticket-com'),esc_html($vrole));
				echo '</label></th><td>';
				...
				echo '>';
				echo '<p class="description">' . esc_html__('If left empty user, will be directed to the login page','wp-ticket-com') . '</p>';
				echo '</td></tr>';
			}
		}
		echo '</tbody></table>';
